...
def kill_halted_tasks_function(**context):
    ...
    logging.info(
        "process_killed_email_addresses: " + str(PROCESS_KILLED_EMAIL_ADDRESSES)
    )
    ...
    for line in search_output_filtered:
        ...
        logging.info(
            "Execution Date to Search For: " + str(execution_date_to_search_for)
        )
        ...
        if DEBUG:
            ...
            for dag in session.query(DagModel).all():
                logging.info(
                    "DEBUG: dag: " + str(dag) + ", dag.is_active: " + str(dag.is_active)
                )
            logging.info("")
        logging.info(
        ...
        dag = (
            ...
            .filter(DagModel.dag_id == process["airflow_dag_id"])
            .first()
        )
        ...
        if DEBUG:
            ...
            for dag_run in (
                ...
                .filter(DagRun.state.in_(dag_run_relevant_states))
                .all()
            ):
                logging.info(
                    "DEBUG: dag_run: "
        ...
        if DEBUG:
            ...
            for task_instance in (
                ...
                .filter(TaskInstance.state.in_(task_instance_relevant_states))
                .all()
            ):
                logging.info(
                    "DEBUG: task_instance: "
    ...
    if ENABLE_KILL:
        ...
        if len(processes_to_kill) > 0:
            ...
            context["ti"].xcom_push(
                key="kill_halted_tasks.processes_to_kill", value=processes_to_kill
            )
